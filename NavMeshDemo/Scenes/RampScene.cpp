...
void RampScene::RegisterMapBarycentricSubDivisionEntity()
{
	...
	m_barycentricSubdividedModel->Mesh = SubDivideBarycentric(m_baseMesh->Mesh);
...
void RampScene::RegisterMapUpperSurfaceModel(const dsr::ModelConfiguration& mapUpperSurface)
{
	...
	std::shared_ptr<TransformComponent> transform = m_sceneManager->AddComponent<TransformComponent>(m_sceneId, m_upperSurfaceEntity);
...
void RampScene::RegisterStartEndMarkerEntities()
{
	...
	XMMATRIX surfaceSubDividedTransformationMatrix = XMMatrixTranslationFromVector(surfaceSubDividedTransform->GetPosition());
...
void RampScene::RegisterPathEntity()
{
	...
	std::shared_ptr<TransformComponent> surfaceTransform = m_sceneManager->GetComponentFrom<TransformComponent>(m_sceneId, m_upperSurfaceEntity);
	XMVECTOR surfacePosition = surfaceTransform->GetPosition();
	XMVectorSetY(surfacePosition, XMVectorGetY(surfacePosition) + 1.0f);
	transform->SetPosition(surfacePosition);
